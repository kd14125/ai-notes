...
- 输出数学公式时，默认使用 Markdown 数学公式语法让公式在对话中直接渲染为可见形式：行内公式使用 `$...$`，独立公式使用 `$$...$$`。除非用户明确要求 LaTeX 源码、可复制代码或排版调试，禁止把数学公式放进 `latex`、`tex` 或普通代码块中展示。
- 每个(cmd或者powershell)会话窗口第一次输出之前必须先运行:
```powershell
# 在 PowerShell 中执行（仅需一次）
$OutputEncoding = [Console]::OutputEncoding = [Text.UTF8Encoding]::new ($false)
```
- 回答必须使用(ps-utf8-io)这个skill
...
- 用中文回答，优先解决核心问题
- 直接进入 ultrathink 模式，如果分析代码问题和修复bug，启用sequential-thinking
...
- Bash 运行在 `D:\git\Git\bin`（Git Bash）
...
# 推荐：直接在命令中使用绝对路径
pnpm test C:/Users/zzz/Projects/myapp/tests
```
...
#在windows上检查 Git Bash 环境
where bash #应返回：D:\git\Git\bin\bash.exe
```
...
处理 Word、PDF、PPT、Excel 等文档任务时，必须隔离过程文件，避免污染原始文档目录或项目主目录。

- **固定过程目录**：所有过程文件必须放到 `H:/codex-temp/codex-doc-work/<workspace-name>/<task-id>/`
- **目录创建要求**：如果 `H:/codex-temp` 不存在，必须先创建它，再创建具体任务目录
- **禁止位置**：禁止把过程文件放到 `C:` 盘 `%TEMP%`、项目主目录或原始文档所在目录
- **过程文件范围**：原始备份、临时 DOCX/PDF、OOXML 解包目录、页面截图、视觉检查图片、日志、Markdown 中间稿、导出验证用文件等都属于过程文件
- **最终目录规则**：最终目录只保留最终可用文件，例如 `_修订版.docx`、`_最终版.pdf`、`_检查报告.md`
- **必要文件保留规则**：清理或归档目录时，必须保留能让项目继续运行、编译或复现的必要文件和配置；只归档旧版本、重复替代输出、临时渲染文件、日志、缓存、辅助编译产物和明确不再使用的备份。对于 LaTeX 项目，`main.tex`、图片目录、被源码引用的 `.cls`/`.sty`/`.bib`/`.bst`、本地编译配置（如 `.vscode/`）以及用户指定的最终 PDF 不得移动到归档目录，除非用户明确指定。
- **完成报告要求**：任务完成报告必须列出最终文件路径、过程目录路径、是否清理过程文件
- **保留限制**：未经用户明确要求，不在最终目录保留备份、截图、临时 PDF 或其他中间文件
- **长期保留确认**：如需长期保留过程文件，必须说明原因并征求用户确认
...
- 当在对话消息中解释数学模型、优化问题、推导过程、约束条件或仿真公式时，优先使用可直接渲染的 Markdown 数学公式。
- 不要把数学公式放进 `latex`、`tex` 或普通代码块中，除非用户明确要求“给我 LaTeX 源码”。
- 行内公式使用 `$...$`，独立公式使用 `$$...$$`。
- 多行优化问题、矩阵、推导链条等使用 `aligned`、`cases`、`bmatrix` 等数学环境，并直接渲染在对话中。
- 解释优化问题时，优先采用“公式 + 含义说明”的格式：先给渲染后的公式，再说明目标函数、优化变量、约束条件和物理意义。
- 只有在用户明确要求复制到论文、Markdown、LaTeX 文件中时，才额外提供源码形式。

### 图片输出

- 当对话消息中需要展示本地图片或生成图片时，必须同时提供两种形式：
  1. 直接显示图片内容，使用 Markdown 图片语法：`![图片说明](绝对路径)`
  2. 提供可点击的文件链接，使用 Markdown 链接语法：`[文件名](绝对路径)`
- 图片路径必须使用绝对路径，不能只给相对路径或纯文本路径。
- 如果图片属于论文、报告或仿真结果，需要在图片前后简要说明该图对应的图号、用途和能支撑的结论。
- 如果一次输出多张图片，每张图片都按“图片预览 + 可点击链接 + 简短说明”的格式分别列出。
- 如果图片尚未生成或无法预览，需要明确说明原因，并提供已生成文件的路径或下一步处理方式。

### 仿真图源文件保存

- 使用 MATLAB 做仿真并生成仿真图时，必须同时保存展示用图片文件和 `.fig` 源文件。
- 使用 Python 做仿真并生成仿真图时，必须同时保存展示用图片文件、`.pkl` Matplotlib Figure 源文件、`.svg` 可编辑矢量图、`.pdf` 矢量图和原始数据文件。
- 同一张仿真图导出的展示图片、源文件、矢量图和原始数据文件必须使用完全一致的基础文件名，仅扩展名不同。
- 同一张仿真图导出的展示图片、源文件、矢量图和原始数据文件必须存储到相同位置，方便后续修改、复现和查找。
## 开发方法
